...
| `supgrade` | Takes a pre/post btrfs snapshot pair (via `snapper`, on `root` and/or `home` if their configs exist), refreshes the pacman mirrorlist with `reflector` (Spain, HTTPS, sorted by rate, mirrors synced within 12h), then runs `pacman -Syyu --noconfirm`, `flatpak update --assumeyes`, [`upgrade-aur`](./upgrade-aur.md), and [`clean-pkgs`](./clean-pkgs.md) — a full system upgrade in one command, undoable with `snapper undochange` if it breaks something. On a non-btrfs system (no `/etc/snapper/configs/root`), it skips snapshotting entirely and just runs the upgrade — see [`btrfs-optimize.sh`](./btrfs-optimize.md) to set the snapshot side up. Authenticates with `sudo -v` once up front and again at each phase boundary, so a long run doesn't re-prompt for a password partway through; `upgrade-aur`'s own per-package PKGBUILD review prompt is left as-is on purpose (a different kind of confirmation — reviewing unaudited third-party build scripts — not upgrade noise to suppress). |

The shell prompt itself is [Starship](../starship)'s job (`starship init fish | source` in `config.fish`), not a fish function here.
